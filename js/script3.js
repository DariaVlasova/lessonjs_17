...
let amountOfPrice = dataList.reduce((prevSum, site) => prevSum + site.price, 0);
document.write(`1)Загальна вартість  :${amountOfPrice}<br>`);
// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
let quantityOfProducts = dataList.reduce(
  (prevSum, products) =>
    products.price < products.old_price ? prevSum + 1 : prevSum,
  0
);
0;
document.write(
  `2)кількість товарів, у яких ціна зменшилась  :${quantityOfProducts}<br>`
);
//3) Товари, які доступні (sell_status:"available")
let availableProducts = dataList.reduce(
  (prevSum, products) =>
    products.sell_status === "available" ? prevSum + 1 : prevSum,
  0
);
0;
document.write(`2)кількість товарів, які доступні  :${availableProducts}<br>`);
//4) сформувати новий
//       список об”єктів тільки доступних для продажу товарів, які міститимуть
//      тільки ідентифікатор товару (id), нову ціну (price), стару ціну
//     (old_price), та ціну у доларах (usd_price)
function getSomePositions({ id, price, old_price, usd_price }) {
  return { id, price, old_price, usd_price };
}
const arrayOfAvailable = [];
for (const list of dataList) {
  if (list.sell_status === "available") {
    arrayOfAvailable.push(getSomePositions(list));
  }
}
for (const item of arrayOfAvailable) {
  document.write(`${JSON.stringify(item)}<br>`);
}
//варіант2
let listOfAvailable = dataList.filter((el) => el.sell_status === "available");

const listOfProducts = listOfAvailable.map(
